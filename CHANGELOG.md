...
**涉及文件：**
- `js/wordbank.js` — 新增 9A（370词），5A-6B 以精校版 TXT 替换
- `assets/parse_9a_v4.py` — 9A 精确解析脚本（CJK分割法）
- `assets/replace_5a6b.py` — 5A-6B 批量替换脚本

#### 词库更新
- **9A 新增**：8单元 370词，CJK字符分割法精确提取，无英文混入中文
- **5A-6B 精校替换**：以精校版 TXT 为准完整替换 OCR 旧数据
  - 5A：203→148词（去掉多余词组例句）
  - 5B：150→153词（整体替换，原数据匹配率仅44%）
  - 6A：147→149词（微调几处差异）
  - 6B：113→112词（微调几处差异）
- 5A-new / 6A-new（2024新版）保持不变，之前已核对无误

---

### [v5] 英语默写全面重构 + 7A-8B 词库扩充

**涉及文件：**
- `english.html` — 全面重构（控件、生成逻辑、CSS）
- `js/wordbank.js` — 新增 7A/7B/8A/8B 数据（1696词），清理脏数据
- `docs/english_v2_demo.html` — 新建样张页面
- `assets/` — 新增 TXT/PDF 源材料、wordbank 备份

#### 英语默写重构
- **删除「题数」「页数」按钮**，改为 50题/页固定分页
- **删除「混合」模式**，只保留「中→英」「英→中」
- **新增统计信息条**：已选单元数 / 总词数 / 需几张纸 / 模式
- 单词**按课本顺序**输出，不再 shuffle 打乱
- **单元分隔标题**：每个 Unit 开头插入横跨两列的蓝色分隔栏
- 2 列布局（每行2个单词），横线占满剩余空间
- 提示词字体加粗，长翻译自动缩小字号
- 清理了旧 CSS 中的冲突样式（第二套 .word-item 定义）

#### 词库扩充
- 新增 7A/7B/8A/8B 共 32 个单元、1696 条词
- 从教材 TXT → MD → wordbank.js 完整转换流程
- 清理了 2.5 万个退格符和混入的说明文字
