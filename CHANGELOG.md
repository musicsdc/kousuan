...
**涉及文件：**
- `kousuan.html` — 二年级 100 以内加减约束、乘加运算难度调整、手机打印列数修正
- `PROGRESS.md` — 更新进度

#### 口算练习（kousuan.html）
- **二年级 100 以内加减不再出现两个个位数**：`genAddSub` 增加 `requireTwoDigit` 参数，二年级题型至少有一个加数/被减数 ≥10
- **二年级乘加运算难度调整**：两个乘数范围 1-9 → 2-9，避免 1 出现频率过高（如 `1 × 1 + 1`）
- **手机打印列数修正**：新增 `@media print and (max-width: 700px)` 规则，覆盖手机响应式 3 列布局，打印时强制按桌面端 4/5 列布局，避免列宽不足导致换行
