...
for line in raw.strip().split('\n'):
    ...
    if m:
        if current_unit and current_words:
            units.append({'unit': current_unit, 'words': current_words})
    ...
    if not m:
        ...
        if m2:
            en = m2.group(1).strip()
            zh = m2.group(2).strip()
            zh = re.sub(r'^[a-z]+\.\s*', '', zh)
            zh = re.sub(r'^[a-z]+\.\s+&\s+[a-z]+\.\s*', '', zh)
    ...
    zh = re.sub(r'^[a-z]+\.\s*', '', zh)
    zh = re.sub(r'^[a-z]+\.\s+&\s+[a-z]+\.\s*', '', zh)
    zh = re.sub(r'^linking\s+v\.\s*', '', zh)
    zh = re.sub(r'^exclam\.\s*', '', zh)
    current_words.append({'en': en, 'zh': zh, 'page': page})

if current_unit and current_words:
    units.append({'unit': current_unit, 'words': current_words})

# 修正 U1/U2 分隔：U1=页码7-18, U2=页码20-32
# 定义各单元页码范围
unit_ranges = {
    'Unit 1': (0, 18),   # 7-18
    'Unit 2': (20, 32),  # 20-32
    ...
    'Unit 5': (65, 76),  # 65-76
    'Unit 6': (79, 90),  # 79-90
    'Unit 7': (93, 104), # 93-104
    'Unit 8': (107, 118),# 107-118
}
...
for u in units:
    for w in u['words']:
        all_words.append({'unit': u['unit'], 'en': w['en'], 'zh': w['zh'], 'page': w.get('page', 0)})
...
for uname in ['Unit 1', 'Unit 2', 'Unit 3', 'Unit 4', 'Unit 5', 'Unit 6', 'Unit 7', 'Unit 8']:
    ...
    units.append({'unit': uname, 'words': [{'en': w['en'], 'zh': w['zh']} for w in fixed[uname]]})

total = sum(len(u['words']) for u in units)
print(f'9A: {len(units)} 单元, {total} 词')
for u in units:
...
out = r'C:\微云同步助手\82127972\kousuan\assets\wordbank_9a_fragment.js'
with open(out, 'w', encoding='utf-8') as f:
    f.write('// 9A 单词数据（译林版）\n')
    f.write('// 由 parse_9a.py 解析生成\n\n')
    f.write('\n'.join(lines))
    f.write('\n')
