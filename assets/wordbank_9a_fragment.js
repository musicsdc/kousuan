// 9A 单词数据（译林版）
// 由 parse_9a.py 解析生成

  "9A": {
    label: "九年级上册",
    semester: "上册",
    grade: 9,
    units: [
      {
        unit: "Unit 1",
        words: [
          { en: "come", zh: "up with 想出,找到(答案)" },
          { en: "keep", zh: "... in (good) order 保持整齐" },
          { en: "neither", zh: "... nor ... 既不...也不..." },
          { en: "at", zh: "all times 总是,随时,永远" },
          { en: "either", zh: "... or ... 要么...要么...,不是...就是..." },
          { en: "decide", zh: "on 决定,选定" },
          { en: "take", zh: "on 承担(责任),决定做" },
          { en: "as", zh: "a matter of fact 事实上" },
          { en: "let", zh: "sb down 使失望" },
          { en: "pass", zh: "sth on to sb 传给某人" },
          { en: "point", zh: "out 指出" },
          { en: "pay", zh: "off 成功,奏效" },
          { en: "stick", zh: "to 坚持" },
          { en: "under", zh: "stress 在压力之下,承受压力" },
          { en: "mobile", zh: "phone n. 手机" },
          { en: "stress", zh: "sb out 使极度焦虑,使压力大" },
          { en: "hear", zh: "of 听说" },
          { en: "risk", zh: "sth/doing sth 冒...的风险" },
          { en: "go", zh: "through 经历,遭受" },
          { en: "stand", zh: "up for sb/sth 支持,维护" },
          { en: "get", zh: "sb down 使悲伤(沮丧、忧郁等)" },
          { en: "organized", zh: "有条理的" },
          { en: "order", zh: "条理;顺序" },
          { en: "explain", zh: "& vi. 解释" },
          { en: "careless", zh: "粗心的" },
          { en: "neither", zh: "也不; det. & pron. 两者都不" },
          { en: "nor", zh: "& adv. 也不" },
          { en: "accountant", zh: "会计" },
          { en: "suppose", zh: "& vi. 认为,推断;假设" },
          { en: "social", zh: "好交际的;社交的" },
          { en: "involve", zh: "包含,需要;牵涉" },
          { en: "impress", zh: "& vi. 使留下深刻的好印象" },
          { en: "imagination", zh: "想象力" },
          { en: "manager", zh: "经理" },
          { en: "quit", zh: "& vi. 离开,离任" },
          { en: "suit", zh: "适合" },
          { en: "railway", zh: "铁路;铁路系统" },
          { en: "mention", zh: "提到,说到,写到" },
          { en: "standard", zh: "标准; adj. 标准的" },
          { en: "tough", zh: "艰难的" },
          { en: "passenger", zh: "乘客" },
          { en: "exactly", zh: "(答语)完全正确;准确地" },
          { en: "personality", zh: "性格,个性" },
          { en: "improvement", zh: "改进,改善" },
          { en: "challenge", zh: "挑战" },
          { en: "winner", zh: "获胜者" },
          { en: "outgoing", zh: "外向的,友好的" },
          { en: "opposite", zh: "对立的人(或事物)" },
          { en: "kindness", zh: "善良" },
          { en: "matter", zh: "关乎...的事情;问题" },
          { en: "probably", zh: "很可能,大概" },
          { en: "encouragement", zh: "鼓励" },
          { en: "behaviour", zh: "(AmE behavior) 表现方式;行为" },
          { en: "son", zh: "儿子" },
          { en: "remain", zh: "保持不变,仍然是" },
          { en: "confident", zh: "自信的" },
          { en: "likely", zh: "可能; adj. 可能的" },
          { en: "negative", zh: "消极的;有害的;否定的" },
          { en: "lecture", zh: "演讲,讲座" },
          { en: "speaker", zh: "发言者;讲(某种语言)的人" },
          { en: "connection", zh: "关联;联结" },
          { en: "point", zh: "& vi. 指向,指引" },
          { en: "succeed", zh: "成功" },
          { en: "position", zh: "职位;位置" },
          { en: "president", zh: "主席;院长;董事长" },
          { en: "cooperate", zh: "合作" },
          { en: "voice", zh: "表达" },
          { en: "consider", zh: "& vi. 仔细考虑" },
        ]
      },
      {
        unit: "Unit 2",
        words: [
          { en: "be", zh: "in low spirits 情绪低落" },
          { en: "cut", zh: "back on sth 减少,削减,缩减" },
          { en: "teenage", zh: "青少年的,十几岁的" },
          { en: "drive", zh: "迫使;驾驶" },
          { en: "mad", zh: "气愤;疯的" },
          { en: "lately", zh: "最近" },
          { en: "decision", zh: "决定" },
          { en: "patience", zh: "耐心" },
          { en: "stick", zh: "& vt. 粘贴,粘住;刺,插入" },
          { en: "stress", zh: "精神压力,心理负担; v. & vi. (使)焦虑不安,疲惫不堪" },
          { en: "waste", zh: "浪费,滥用" },
          { en: "regret", zh: "懊悔,感到遗憾" },
          { en: "exam", zh: "(= examination /ɪɡˌzæmɪˈneɪʃn/) 考试" },
          { en: "focused", zh: "注意力集中的,目标明确的" },
          { en: "argue", zh: "争论,争吵,争辩" },
          { en: "silly", zh: "傻的,愚蠢的" },
          { en: "normal", zh: "正常的,典型的" },
          { en: "bottom", zh: "底部;底" },
          { en: "dialogue", zh: "对话" },
          { en: "communicate", zh: "& vt. 交流; vi. 沟通" },
          { en: "dictionary", zh: "词典,辞书" },
          { en: "youth", zh: "青年;青春" },
          { en: "stage", zh: "阶段,时期;舞台" },
          { en: "experience", zh: "经历;体验" },
          { en: "rapid", zh: "快速的" },
          { en: "ugly", zh: "丑陋的" },
          { en: "mood", zh: "情绪,心情" },
          { en: "risk", zh: "冒...的风险;冒险做" },
          { en: "adult", zh: "成年人" },
          { en: "mean", zh: "刻薄;吝啬的" },
          { en: "stranger", zh: "陌生人;外地人" },
          { en: "uncomfortable", zh: "不自在的;使人不舒服的" },
          { en: "midterm", zh: "期中的" },
          { en: "tight", zh: "紧的" },
          { en: "overweight", zh: "超重的" },
          { en: "bully", zh: "恐吓,伤害,胁迫; n. 恃强凌弱之人,恶霸" },
          { en: "screen", zh: "屏幕" },
          { en: "bedtime", zh: "就寝时间" },
          { en: "pain", zh: "痛苦;疼痛" },
          { en: "shame", zh: "羞耻;让人遗憾的事" },
        ]
      },
      {
        unit: "Unit 3",
        words: [
          { en: "household", zh: "name n. 家喻户晓的人物" },
          { en: "be", zh: "diagnosed with 被诊断出患有...(疾病)" },
          { en: "rob", zh: "sb of sth 剥夺(某人所需或应得之物)" },
          { en: "stand", zh: "in sb's way 妨碍某人,阻止某人" },
          { en: "make", zh: "it (to) 获得成功" },
          { en: "fair", zh: "play n. 公平竞赛,公平办事" },
          { en: "live", zh: "through sth 经历(灾难等)而幸存" },
          { en: "seed", zh: "种子;起因" },
          { en: "dancer", zh: "舞蹈演员,跳舞者" },
          { en: "recognize", zh: "(= recognise) 赞赏,公认;认识,认出" },
          { en: "contribution", zh: "贡献" },
          { en: "overcome", zh: "克服,解决" },
          { en: "suffering", zh: "痛苦,苦难" },
          { en: "household", zh: "家庭,一户" },
          { en: "inspiration", zh: "鼓舞人心的人(或事物);灵感" },
          { en: "further", zh: "促进,增进" },
          { en: "diagnose", zh: "诊断" },
          { en: "rob", zh: "掠夺" },
          { en: "shortly", zh: "不久" },
          { en: "muscle", zh: "肌肉" },
          { en: "yet", zh: "然而,但是" },
          { en: "surprisingly", zh: "使人惊奇地" },
          { en: "attitude", zh: "态度" },
          { en: "despite", zh: "尽管,即使" },
          { en: "universe", zh: "宇宙" },
          { en: "continued", zh: "连续不断的" },
          { en: "scientific", zh: "科学的" },
          { en: "audience", zh: "读者,观众,听众" },
          { en: "highly", zh: "很,非常" },
          { en: "whenever", zh: "无论何时" },
          { en: "achievement", zh: "成就" },
          { en: "however", zh: "不管多么" },
          { en: "musician", zh: "音乐家" },
          { en: "death", zh: "死亡" },
          { en: "shoulder", zh: "肩膀" },
          { en: "coach", zh: "教练" },
          { en: "pretty", zh: "相当" },
          { en: "knee", zh: "膝,膝盖" },
          { en: "challenging", zh: "有挑战性的" },
          { en: "cheers", zh: "再见;干杯" },
          { en: "fair", zh: "公平的,公正的" },
          { en: "Irish", zh: "爱尔兰(人)的,爱尔兰语的" },
          { en: "gymnast", zh: "体操运动员" },
          { en: "compete", zh: "参加比赛;竞争" },
          { en: "injure", zh: "伤害,使受伤" },
          { en: "performance", zh: "表现;表演" },
          { en: "will", zh: "意志,毅力;意愿" },
          { en: "runner", zh: "赛跑运动员,跑步者" },
          { en: "semi-final", zh: "半决赛" },
          { en: "whatever", zh: "& pron. 无论什么,不管什么" },
          { en: "diary", zh: "日记" },
          { en: "victory", zh: "胜利" },
          { en: "perform", zh: "做,执行; v. & vi. 表演" },
          { en: "disappoint", zh: "使...失望" },
        ]
      },
      {
        unit: "Unit 4",
        words: [
          { en: "a", zh: "couple of 几个;一对" },
          { en: "dig", zh: "up 掘地;挖掘出" },
          { en: "dig", zh: "out 挖掘出;发掘" },
          { en: "expect", zh: "sth of sb 期待某人做某事(或具备某种能力、品质)" },
          { en: "devote...to...", zh: "把...奉献给..." },
          { en: "make", zh: "up one's mind 下定决心" },
          { en: "cactus", zh: "仙人掌" },
          { en: "lotus", zh: "莲属植物;莲花图案" },
          { en: "self", zh: "自我;本来面目" },
          { en: "peanut", zh: "花生" },
          { en: "unused", zh: "未用过的" },
          { en: "pity", zh: "遗憾" },
          { en: "dig", zh: "& vi. 挖,掘,凿" },
          { en: "celebration", zh: "庆祝;庆典" },
          { en: "oil", zh: "食用油;石油" },
          { en: "unlike", zh: "不像" },
          { en: "stem", zh: "茎,梗,柄" },
          { en: "bear", zh: "结(果实),开(花);生(孩子)" },
          { en: "attractive", zh: "吸引人的" },
          { en: "compare", zh: "比较,对比" },
          { en: "hidden", zh: "隐藏的,隐匿的" },
          { en: "society", zh: "社会" },
          { en: "helpfulness", zh: "乐于助人" },
          { en: "purpose", zh: "重要意义;目的,意图" },
          { en: "satisfied", zh: "满意的" },
          { en: "boss", zh: "老板,上司" },
          { en: "dentist", zh: "牙科医生" },
          { en: "wife", zh: "(pl. wives) 妻子" },
          { en: "stamp", zh: "邮票" },
          { en: "daughter", zh: "女儿" },
          { en: "passage", zh: "章节;段落;乐段" },
          { en: "devote", zh: "献身,致力,专心" },
          { en: "research", zh: ", vi. & n. 研究" },
          { en: "house", zh: "收藏,安置" },
          { en: "overtourism", zh: "过度旅游" },
          { en: "graduate", zh: "& vt. (大学)毕业" },
          { en: "digitalize", zh: "(= digitalise) 使(信息)数字化" },
          { en: "serve", zh: "能满足...的需要; vt. & vi. 服务;提供" },
          { en: "condition", zh: "状态,状况" },
          { en: "contribute", zh: "& vt. 贡献,增进,促成" },
          { en: "judge", zh: "& vi. 判断,认为; vt. 审判" },
          { en: "amazed", zh: "大为惊奇" },
        ]
      },
      {
        unit: "Unit 5",
        words: [
          { en: "familiar", zh: "with sth 通晓,熟悉" },
          { en: "make", zh: "up 创作,编写;编造" },
          { en: "keyboard", zh: "键盘" },
          { en: "bow", zh: "琴弓;弓" },
          { en: "string", zh: "弦;细绳" },
          { en: "guitar", zh: "吉他" },
          { en: "strength", zh: "强度,力度;力量" },
          { en: "pear-shaped", zh: "梨形的" },
          { en: "musical", zh: "音乐的" },
          { en: "instrument", zh: "乐器;仪器" },
          { en: "boundary", zh: "边界" },
          { en: "composer", zh: "作曲家" },
          { en: "unique", zh: "独一无二的" },
          { en: "earn", zh: "赢得,博得; vt. & vi. 赚得,挣钱" },
          { en: "familiar", zh: "熟悉的" },
          { en: "worldwide", zh: "在世界各地; adj. 影响全世界的" },
          { en: "simply", zh: "仅仅" },
          { en: "flow", zh: "流,流动" },
          { en: "eastern", zh: "东方国家的;东方的" },
          { en: "bell", zh: "钟(声);电铃(声)" },
          { en: "talented", zh: "有天赋的" },
          { en: "mixture", zh: "混合" },
          { en: "press", zh: "新闻界;平面媒体" },
          { en: "brain", zh: "脑;智力" },
          { en: "ticket", zh: "票,入场券" },
          { en: "concert", zh: "音乐会" },
          { en: "rather", zh: "相当,在某种程度上" },
          { en: "country", zh: "乡村" },
          { en: "jazz", zh: "爵士乐" },
          { en: "rock", zh: "摇滚乐" },
          { en: "drum", zh: "鼓" },
          { en: "cowboy", zh: "牛仔" },
          { en: "African", zh: "非洲(人)的" },
          { en: "American", zh: "美国人,美洲人" },
          { en: "attend", zh: "& vi. 出席,参加;定期去(某处)" },
          { en: "quick", zh: "迅速的" },
          { en: "jasmine", zh: "茉莉" },
          { en: "flute", zh: "长笛" },
          { en: "range", zh: "一系列;类;范围" },
          { en: "gentle", zh: "柔和的;温柔的" },
          { en: "download", zh: "下载" },
          { en: "calming", zh: "令人平静的" },
        ]
      },
      {
        unit: "Unit 6",
        words: [
          { en: "date", zh: "back (to...) 追溯到" },
          { en: "give", zh: "it a try 试一试" },
          { en: "concentrate", zh: "on 集中注意力(做某事)" },
          { en: "take", zh: "pleasure in 从...中获得乐趣" },
          { en: "slow", zh: "down (使)慢下来" },
          { en: "under", zh: "pressure 承受着压力;被迫" },
          { en: "up", zh: "close 很近地,在近处" },
          { en: "in", zh: "general 大体上;总的说来" },
          { en: "a", zh: "great deal 大量,很多" },
          { en: "calligrapher", zh: "书法家" },
          { en: "bottled", zh: "瓶装的" },
          { en: "calligraphy", zh: "书法" },
          { en: "tip", zh: "尖,尖端" },
          { en: "final", zh: "最终的,最后的" },
          { en: "stroke", zh: "笔画" },
          { en: "appear", zh: "出现" },
          { en: "doorway", zh: "门口,门道" },
          { en: "still", zh: "静止的,平静的,安静的" },
          { en: "concentrate", zh: "& vt. 集中(注意力); v. 使...集中" },
          { en: "pleasure", zh: "快乐,满足" },
          { en: "undoubtedly", zh: "无疑地" },
          { en: "character", zh: "文字" },
          { en: "artistic", zh: "艺术的" },
          { en: "characteristic", zh: "特征" },
          { en: "breath", zh: "呼吸的空气" },
          { en: "pressure", zh: "心理压力;压力" },
          { en: "pride", zh: "自豪,骄傲" },
          { en: "beautifully", zh: "漂亮地" },
          { en: "technique", zh: "技巧,技艺" },
          { en: "unfamiliar", zh: "不熟悉的" },
          { en: "basket", zh: "篮,篓,筐" },
          { en: "perhaps", zh: "可能,也许" },
          { en: "general", zh: "一般的;总的" },
          { en: "brushwork", zh: "画法,笔触" },
          { en: "similarity", zh: "相似性" },
          { en: "present", zh: "展现" },
          { en: "sometime", zh: "在某时" },
          { en: "expert", zh: "专家" },
        ]
      },
      {
        unit: "Unit 7",
        words: [
          { en: "based", zh: "upon/on 以...为基础(或根据)" },
          { en: "pass", zh: "away 去世" },
          { en: "a", zh: "number of 一些" },
          { en: "rush", zh: "hour n. 交通高峰期" },
          { en: "steal", zh: "the show 吸引更多的注意,抢风头" },
          { en: "bring", zh: "sb/sth to life 使更有趣(生动)" },
          { en: "come", zh: "to life 有了生命,活了过来" },
          { en: "action", zh: "(= action film) 动作片" },
          { en: "comedy", zh: "喜剧" },
          { en: "cartoon", zh: "卡通片;漫画" },
          { en: "documentary", zh: "纪录片,纪实广播(或电视)节目" },
          { en: "romance", zh: "爱情故事;传奇故事" },
          { en: "all-time", zh: "空前的,一向的" },
          { en: "loss", zh: "去世;丧失,损失" },
          { en: "actress", zh: "女演员" },
          { en: "humanitarian", zh: "人道主义者" },
          { en: "ballet", zh: "芭蕾舞" },
          { en: "charm", zh: "魅力" },
          { en: "insist", zh: "& vi. 坚持说;坚决要求" },
          { en: "based", zh: "基于" },
          { en: "major", zh: "主要的,重要的" },
          { en: "mark", zh: "是...的迹象,表明;标示" },
          { en: "career", zh: "事业;职业" },
          { en: "princess", zh: "公主" },
          { en: "appearance", zh: "演出;出现" },
          { en: "angel", zh: "天使" },
          { en: "radio", zh: "无线电广播;收音机" },
          { en: "discover", zh: "发现" },
          { en: "cancer", zh: "癌症" },
          { en: "peacefully", zh: "平静地" },
          { en: "star", zh: "主演,担任主角" },
          { en: "aid", zh: ", vi. & vt. 援助,帮助" },
          { en: "tireless", zh: "不知疲倦的" },
          { en: "actor", zh: "演员" },
          { en: "park", zh: "& vi. 停(车)" },
          { en: "script", zh: "剧本;脚本" },
          { en: "storyteller", zh: "故事(或小说)作者" },
          { en: "mistake", zh: "误会,误解" },
          { en: "steal", zh: "偷,窃取" },
          { en: "supporting", zh: "次要的,配角的" },
          { en: "kung fu", zh: "功夫" },
          { en: "handsome", zh: "英俊的,帅气的" },
          { en: "inspiring", zh: "激励的,鼓舞人心的" },
          { en: "price", zh: "价格;代价" },
          { en: "scriptwriter", zh: "编剧" },
          { en: "director", zh: "导演" },
          { en: "film-maker", zh: "制片人" },
          { en: "film", zh: "& vt. 拍摄" },
          { en: "shoot", zh: "& vi. 拍摄" },
          { en: "editor", zh: "剪辑员;编辑" },
          { en: "post-production", zh: "后期制作" },
          { en: "world-class", zh: "世界级的,世界一流的" },
          { en: "wrestler", zh: "摔跤运动员" },
          { en: "touching", zh: "感人的" },
          { en: "fighter", zh: "斗士" },
          { en: "elder", zh: "年纪较长的" },
        ]
      },
      {
        unit: "Unit 8",
        words: [
          { en: "short", zh: "story n. 短篇小说" },
          { en: "take", zh: "one's time 从容不迫,慢慢来" },
          { en: "make", zh: "sense 有意义,讲得通" },
          { en: "have", zh: "(...) to do with 与...有关" },
          { en: "run", zh: "out of 用完,耗尽" },
          { en: "show", zh: "up 出现,露面" },
          { en: "hurry", zh: "up 赶快,急忙(做某事)" },
          { en: "classic", zh: "最优秀的;典型的;典雅的" },
          { en: "branch", zh: "树枝" },
          { en: "against", zh: "倚,紧靠;以...为背景" },
          { en: "bedside", zh: "床边" },
          { en: "anger", zh: "愤怒,怒气" },
          { en: "apartment", zh: "公寓套房" },
          { en: "masterpiece", zh: "杰作" },
          { en: "widely", zh: "普遍地,广泛地" },
          { en: "fool", zh: "傻瓜" },
          { en: "remain", zh: "剩余,继续存在;逗留" },
          { en: "fair", zh: "展销会;集市" },
          { en: "seat", zh: "(使)就座,坐" },
          { en: "act", zh: "(戏剧等)一幕;行为;表演" },
          { en: "throne", zh: "宝座;王位" },
          { en: "image", zh: "意象;图像" },
          { en: "lasting", zh: "持久的" },
          { en: "unable", zh: "未能,无法" },
          { en: "enemy", zh: "敌人" },
          { en: "writing", zh: "著作,作品" },
          { en: "mirror", zh: "镜子" },
          { en: "knock", zh: "敲门声,敲击声" },
          { en: "doorbell", zh: "门铃" },
          { en: "ring", zh: "发出铃声; vt. & vi. (使)发出钟声" },
        ]
      },
    ]
  },
